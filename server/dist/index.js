...
app.post("/api/events", (req, res) => {
    try {
        ...
        if (organizerId !== undefined && typeof organizerId !== "string") {
            return res.status(400).json({
                error: "organizerId must be a string when provided",
            });
        }
        const fallbackAdmin = (0, store_1.getUserByEmail)("admin@local-events.app");
        if (!fallbackAdmin) {
            return res
                .status(500)
                .json({ error: "Default admin account is not available" });
        }
        const resolvedOrganizerId = organizerId ?? fallbackAdmin.id;
        const created = (0, businessLogic_1.createOrganizerEvent)(resolvedOrganizerId, eventInput);
